...
var yearInt;
var toPlayGame = true;
var toStartBattleLoop;
var battlePointOfPlayer;
var battlePointofEnemy;

var battleLoop = function () {
    alert("Biff is going to attack with you with his cane.");
    var hitOrMiss = Math.floor(Math.random() * 2);
    if (hitOrMiss === 0) {
        alert("Bliff has a miss.");
    }

    else {
        alert("Bliff has hit you.");
        battlePointofEnemy = battlePointofEnemy + 1;
    }

    alert("Now, you are going to attack Biff.");
    hitOrMiss = Math.floor(Math.random() * 2);

    if (hitOrMiss === 0) {
        alert("You has a miss.");
    }

    else {
        alert("You have hit Biff.");
        battlePointOfPlayer = battlePointOfPlayer + 1;
    }

    var inputUser = prompt("Do you wish to continue the battle. Yes, No. (Y/N)");
    while (inputUser.toLowerCase() !== "y" && inputUser.toLowerCase() !== "n") {
        inputUser = prompt("Do you wish to continue the battle. Yes, No. (Y/N)");
    }

    if (inputUser.toLowerCase() === "y") {
        toStartBattleLoop = true;
    }

    else {
        toStartBattleLoop = false;
        alert("Battle point of enemy: " + battlePointofEnemy + ". Your battle point: " + battlePointOfPlayer);
    }
}
...
var biff  = function () {
    ...
    if (userInput.toLowerCase() === "s" ) {
        toStartBattleLoop = true;
        battlePointOfPlayer = 0;
        battlePointofEnemy = 0;

        while (toStartBattleLoop) {
            battleLoop();
        }

        if (battlePointOfPlayer > battlePointofEnemy) {
            alert("You have won in the battle. Congrats. Point earned = 1");
            point = point + 1;
            numOfGame = numOfGame + 1;
            promptAnotherGame();
        }
            ...
            alert("You have lost in the battle. Point earned = 0");
            numOfGame = numOfGame + 1;
            promptAnotherGame();
        }
    }
