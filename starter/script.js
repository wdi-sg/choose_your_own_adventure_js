...
var daggerForestQuestOutcomes = function(whatForestQuest){
  if(whatForestQuest%2 === 0){
      ...
      returnHome(playerName);
}

var serpentRiverQuestOutcomes = function(whatRiverQuest){
...
var daggerForestQuest = function(){
  ...
  daggerForestQuestOutcomes();
}

var serpentRiverQuest = function(){
  ...
  serpentRiverQuestOutcomes(whatRiverQuest);
}

var whereToInWilderness = function(name){
    var playerNameInput = cappedPlayerName(name);
...
var gloomhavenSquareQuestOutcomes = function(whatSquareQuest){
...
var sinkingMarketQuestOutcomes = function(marketQuest){
  if(marketQuest === 1){
...
var gloomhavenSquareQuest = function(){
  ...
  gloomhavenSquareQuestOutcomes(whatSquareQuest);
}

var sinkingMarketQuest = function(){
    ...
    sinkingMarketQuestOutcomes(marketQuest);
}

var whereToInTown = function(name){
  var playerNameInput = cappedPlayerName(name);
  ...
  if(whereTown.toLowerCase() === "sinking market"){
    ...
    gloomhavenSquareQuest();
  }
}

var whereToGoGloomhaven = function(name){
  var playerNameInput = cappedPlayerName(name);
  var whereGloomhaven = prompt("Where would you like to wander off to?",
  "town or wilderness");

  while(whereGloomhaven.toLowerCase() !== "town" && whereGloomhaven.toLowerCase() !== "wilderness"){
      alert("Are you lost? " + playerNameInput + " the place you entered doesn't exists, "
      + "please re-enter.");
      var whereGloomhaven = prompt("Where would you like to wander off first?",
      "towns or wilderness")
  }

  if(whereGloomhaven.toLowerCase() === "town"){
      alert("Hallo " + playerNameInput + ", Welcome to the town!");
      whereToInTown(name);
  }
  else if(whereGloomhaven.toLowerCase() === "wilderness"){
      alert(playerNameInput +"! "+ "Look out for dungeons and definitely go explore forgotten ruins!");
      whereToInWilderness(name);
  }
}
...
var whereGloomhaven = whereToGoGloomhaven(playerName);
