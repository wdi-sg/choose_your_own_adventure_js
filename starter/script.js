...
function darkPath() {
	var darkRoom = prompt(playerName + " entered a dark room and felt a breeze coming from the other end of the room but " + playerName + " sensed another presence as well. Search for the Light or run towards the Breeze? (L/B)").toLowerCase();
	while (darkRoom !== "l" && darkRoom !== "b") {
		darkPrompt();
		if (darkRoom === "l") {
			var lightSwitch = prompt("There was another deaf creature at the corner of the room, upon catching sight of " + playerName + " it aggressively lunged towards " + playerName + "." + " Fight back, try to Escape or Go back to previous options? (F/E/G)").toLowerCase();
			while (lightSwitch !== "f" && lightSwitch !== "e" && lightSwitch !== "g") {
				...
				if (lightSwitch === "f") {
					...
					var repeatDarkPath = prompt("Do you want to repeat the Dark room or back to the Beginning? (D/B)").toLowerCase();
...
function whitePath() {
	var whiteDoor = prompt(playerName + " entered a white empty room with a staff on an empty throne and another white door at the end of the room. Grab Staff or head towards the Door? (S/D)").toLowerCase();
	while (whiteDoor !== "s" && whiteDoor !== "d") {
		whitePrompt();
		if (whiteDoor === "s") {
			var sleepingMage = prompt("As " + playerName + " touches the staff, a Mage appears on the throne. The mage appears to be asleep. Wake him up, Steal his staff or Go back to previous options? (W/S/G)").toLowerCase();
			while (sleepingMage !== "w" && sleepingMage !== "s" && sleepingMage !== "g") {
				...
				if (sleepingMage === "w") {
					...
					var repeatWhitePath = prompt("Do you want to repeat the White room or go back to the Beginning? (W/B)").toLowerCase();
